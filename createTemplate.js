  ...
  if (Array.isArray(results)) {
    ...
    const summaryTable = `
        ...
        ${Object.keys(summary)
          .map(
            (key) =>
              `<li class="summary-content"><span>${key}</span><span>${summary[key].error}</span><span>${summary[key].warning}</span></li>`
          )
          .join('\n\t')}
        </ul>
        `;
    ...
    const detailsTable = `
        ...
        <div class="error-summary ${
          errorObj.errored ? 'errored' : 'valid'
        } clearfix"><span class="filename" title="${errorObj.filesource}">${
              errorObj.filename || getFileName(errorObj.filesource)
            }</span><span class="error-count">Errors: <span>${
              errorObj.errors.error
            }</span></span><span class="warning-count">Warnings: <span>${
              errorObj.errors.warning
            }</span></span></div>
        ${
          errorObj.warnings.length
            ? `<div class="error-list clearfix hidden">
            ${errorObj.warnings
              .map(
                (warning) => `
            <div class="${warning.severity} clearfix">
                ...
                <span class="line">ln: <span>${
                  warning.line
                }</span> col: <span>${warning.column}</span></span>
                <span class="message" title="${warning.text.replace(
                  /[\"]/g,
                  ''
                )}">${warning.text}</span>
            </div>
            ...
            : '<span class="no-error hidden">This file has no errors</span>'
        }
        </li>
        `
          )
        ...
        `;
    return baseHtml
      .replace('{{summaryTable}}', summaryTable)
      .replace('{{detailedList}}', detailsTable);
  }
};
